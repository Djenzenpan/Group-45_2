...
```
*Observations:* 94.9% NULL values. Note that the mean hotel rating (*visitor_hist_starrating*) is specific to an individual (*srch_id*). We can potentially replace null values with a calculated value of the means.
...
```
*Observations:* 94.89% NULL values. Most common mean price per night is 200 USD.
...
*Observations:* 22% NULL values. Second score data appear to be logarithmic. 
*Questions:* Why have a second score? What is it for compared to the first score? (Guido's answer: don't know but don't disregard these features as they may still be useful)
...
*Observations:* 97.4% of values are NULL. Of the non-NULL set: 4.2% only Expedia has availablility; 94.6% both Expedia and competitor1 have availability; 1.1% only competitor1 has availability
...
*Observations:* Data ranges from 1 to 40. Histogram has a distinct pattern. TODO: look into why that is so.
